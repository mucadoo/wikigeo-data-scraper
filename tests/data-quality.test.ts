...
describe('Data Quality Checks', () => {
  ...
  it('should not contain <ref> tags in any field', () => {
    ...
    countries.forEach((country: any) => {
      ...
      if (countryStr.includes('<ref')) {
        countriesWithRefs.push(country.name.en);
      }
    });
  ...
  it('should not have "capital" as a name in largestCity unless it is actually named Capital', () => {
    ...
    countries.forEach((country: any) => {
      country.largestCity?.forEach((city: any) => {
  ...
  it('should have flagUrl populated for most countries', () => {
    const countriesMissingFlag = countries.filter((c: any) => !c.flagUrl).map((c: any) => c.name.en);
  ...
  it('should have densityKm2 populated for most countries', () => {
    const countriesMissingDensity = countries.filter((c: any) => c.densityKm2 === null).map((c: any) => c.name.en);
  ...
  it('should not have empty articleId for important fields if they have a name', () => {
    ...
    const fieldsToCheck = ['capital', 'largestCity', 'officialLanguage', 'currency'];
    
    countries.forEach((country: any) => {
      fieldsToCheck.forEach(field => {
        country[field]?.forEach((item: any) => {
